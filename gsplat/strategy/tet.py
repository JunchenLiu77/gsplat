...
from .ops import cut, remove, reset_opa
from typing_extensions import Literal


@dataclass
class TetStrategy(Strategy):
    ...
    def step_post_backward(
        ...
        if (
            ...
            # grow GSs
            n_cut = self._grow_gs(params, optimizers, state, step)
            if self.verbose:
                print(
                    f"Step {step}: {n_cut} GSs cut. "
    ...
    def _grow_gs(
        ...
        is_cut = is_grad_high
        n_cut = is_grad_high.sum().item()

        if n_cut > 0:
            cut(params=params, optimizers=optimizers, state=state, mask=is_cut)
